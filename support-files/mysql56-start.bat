...
ECHO Starting MySQL 5.6... retries %MYSQL_START_RETRIES% wait time %MYSQL_START_WAIT%
IF NOT EXIST %MYSQL_HOME%\bin\mysqld.exe GOTO :ERROR_MYSQL_NOT_FOUND
START "MySQL 5.6 mysqld.exe" /D %MYSQL_HOME% CMD /K "%MYSQL_HOME%\bin\mysqld.exe --defaults-file=%MYSQL_DEFAULTS_FILE% --basedir=%MYSQL_HOME% --port=%MYSQL_PORT% --datadir=%MYSQL_DATADIR% --pid-file=%MYSQL_PID_FILE% --secure-file-priv=%MYSQL_SECURE_FILE_PRIV% --log-error=%MYSQL_LOG_ERROR% & EXIT"
TIMEOUT 5
CALL %TEST_DIR%\support-files\monitor-file.bat "MySQL 5.6 error log" %MYSQL_LOG_ERROR%
